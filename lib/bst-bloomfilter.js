 ...
 */
BinarySearchTreeBloomFilter.prototype.insert = function (key, value, getArray) {
  ...
  if (this.compareKeys(key, this.key) < 0) {
    ...
    if (this.left) {
      return this.left.insert(key, value, getArray);
    } else {
    ...
    if (this.right) {
      return this.right.insert(key, value, getArray);
    } else {
...
BinarySearchTreeBloomFilter.prototype.betweenBounds = function (query, lbm, ubm) {
  ...
  if (lbm(this.key) && ubm(this.key)) { append(res, this.dataArray); }
